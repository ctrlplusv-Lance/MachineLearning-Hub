...
export default function CommentModal({ article, onClose, user }) {
  ...
  useEffect(() => {
    ...
    // Realtime subscription to listen for new comments on this specific article
  ...
  // Logic to separate top-level comments from replies
          ...
          {rootComments.map((comment) => (
            <div key={comment.id} className="space-y-4">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <p className="text-[10px] font-black text-blue-600 mb-1 uppercase tracking-wider">
              ...
              {getReplies(comment.id).map(reply => (
                <div key={reply.id} className="ml-8 bg-blue-50/40 p-3 rounded-2xl border-l-4 border-blue-200">
                   <p className="text-[10px] font-black text-blue-400 mb-1 uppercase tracking-wider">
        ...
        <div className="p-6 border-t bg-slate-50">
          ...
          <div className="flex gap-2">
            <input 
              ...
              // Added text-slate-900 to ensure typed text is black/visible
